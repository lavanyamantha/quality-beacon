    ...
    <div className="dashboard-card col-span-1 md:col-span-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
        {services.map((svc, i) => (
          <motion.div
            key={svc.id}
            className="flex items-center gap-2.5 p-2.5 rounded-md bg-secondary/50 hover:bg-secondary transition-colors min-w-0"
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.04 }}
          >
            <span className={`${healthDot[svc.health]} flex-shrink-0`} />
